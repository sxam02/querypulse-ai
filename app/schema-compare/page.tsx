...
export default function SchemaComparePage() {
  ...
  const [sqlSeparator, setSqlSeparator] = useState<'semicolon' | 'go'>('semicolon');

  // Load configuration and profiles
  useEffect(() => {
    setMounted(true);

    const storedProfiles = localStorage.getItem('querypulse_saved_db_profiles');
    if (storedProfiles) {
      try {
      ...
      localStorage.setItem('querypulse_saved_db_profiles', JSON.stringify(DEFAULT_PROFILES));
      setProfiles(DEFAULT_PROFILES);
    }
  ...
  const handleCreateProfile = () => {
    ...
    setProfiles(updated);
    localStorage.setItem('querypulse_saved_db_profiles', JSON.stringify(updated));
  ...
  const runComparisonWorkflow = async () => {
    ...
    try {
      ...
      for (let i = 0; i < totalSteps; i++) {
        ...
        addLog(`[2/3] [${comp}] Fetching metadata from Source DB...`);
        const srcRes = await fetch('/api/compare-extract', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            dbConfig: sourceDb,
        ...
        if (!srcRes.ok) {
          const errData = await srcRes.json();
          throw new Error(`Source [${comp}] extraction failed: ${errData.error}`);
        }

        const srcData = await srcRes.json();
        sourceSnapshot[comp as keyof DatabaseSchemaSnapshot] = srcData[comp];
        const srcCount = Object.keys(srcData[comp] || {}).length;
        addLog(`[${comp}] Source: Extracted ${srcCount} structures.`);

        let prog = 15 + Math.round(((i * 2 + 1) / (totalSteps * 2)) * 75);
        setCompareProgress(prog);
        await sleep(isDemo ? 350 : 150);

        addLog(`[${comp}] Fetching metadata from Destination DB...`);
        const destRes = await fetch('/api/compare-extract', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            dbConfig: destinationDb,
            component: comp,
            useDemo: isDemo,
            demoTarget: 'destination'
          })
        });

        if (!destRes.ok) {
          const errData = await destRes.json();
          throw new Error(`Destination [${comp}] extraction failed: ${errData.error}`);
        }

        const destData = await destRes.json();
        destSnapshot[comp as keyof DatabaseSchemaSnapshot] = destData[comp];
        const destCount = Object.keys(destData[comp] || {}).length;
        addLog(`[${comp}] Destination: Extracted ${destCount} structures.`);
...
`.trim();
  ...
  return (
    <div style={{
      ...
      {/* ─── Main Content ──────────────────────────────────────────── */}
      <main style={{ flex: 1, maxWidth: 1200, width: '100%', margin: '0 auto', padding: '48px 24px 96px' }}>
              ...
              <div style={{ position: 'relative', width: 120, height: 120 }}>
                <svg style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)' }} viewBox="0 0 36 36">
                  ...
                  <path
                    ...
                    stroke={CB.primary}
                    fill="none"
                    strokeDasharray={`${compareProgress}, 100`}
                    strokeLinecap="round"
